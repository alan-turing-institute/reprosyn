...
def load_census(datapath = '../../datasets/2011-census-microdata/2011-census-microdata-small.csv'):
    
    data = pd.read_csv(datapath)
    data.drop(["Person ID"],axis=1, inplace =True) 
    for col in data.columns: data[col]= data[col].astype('category').cat.codes
    print('census loaded')
     
    return (Dataset(data, Domain.fromdict(get_domain_dict(data))))
...
if __name__ == '__main__':
    ...
    if args.num_marginals is not None:
        workload = [workload[i] for i in rng.choice(len(workload), args.num_marginals, replace=False)]
    ...
    synth.df.to_csv(os.path.join(output_dir, filename))
